...
func newUpdateCmd() *cobra.Command {
 cmd := &cobra.Command{
  ...
  Short: "Run dependabot update locally (auto-detect Go 1.27 STABLE)",
  Long: `Detecta automáticamente: go.mod, package.json, requirements.txt, Cargo.toml`,
  ...
  RunE: func(cmd *cobra.Command, args []string) error {
   ...
   if ecosystem == "" {
    ecosystem = detectEcosystem(absDir)
   }

   verbose, _ := cmd.Flags().GetBool("verbose")
   if verbose || ecosystem!= "" {
    fmt.Printf(">> Diamond STABLE auto-detect\n>> Ecosystem: %s\n>> Dir: %s\n", ecosystem, absDir)
   }
